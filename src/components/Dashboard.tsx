...
function calculateTaskCounts(tasks: Task[], dateRange: DateRange): {
  ...
  const tasksCreatedInRange = tasks.filter(task => {
    try {
      const taskDate = parseISO(task.date);
      return isWithinInterval(taskDate, { 
        start: startOfDay(dateRange.start), 
  ...
  const created = tasksCreatedInRange.length;
  const completed = tasksCreatedInRange.filter(task => task.isCompleted === true).length;
...
function analyzeProcesses(tasks: Task[]): { 
  ...
  const tasksWithProcesses = tasks.filter(task => task.processSummary);
  const processFrequency: Record<string, number> = {};
  const processDurations: Record<string, number[]> = {};
  
  tasksWithProcesses.forEach(task => {
    if (task.processSummary) {
      const normalizedProcess = task.processSummary.toLowerCase().trim();
      processFrequency[normalizedProcess] = (processFrequency[normalizedProcess] || 0) + 1;
      
      if (!processDurations[normalizedProcess]) {
        processDurations[normalizedProcess] = [];
      }
      processDurations[normalizedProcess].push(safeCalculateDuration(task));
    }
  });
  
  const uniqueCount = Object.keys(processFrequency).length;
  const repeatableCount = Object.values(processFrequency).filter(count => count >= 2).length;
  
  let totalDuration = 0;
  let processCount = 0;
  
  Object.entries(processDurations).forEach(([_, durations]) => {
    const avgDuration = durations.reduce((sum, duration) => sum + duration, 0) / durations.length;
    totalDuration += avgDuration;
    processCount++;
  });
  
  const averageTimePerProcess = processCount > 0 ? Math.round(totalDuration / processCount) : 0;
  
  return {
    uniqueCount,
    repeatableCount,
    averageTimePerProcess,
    processFrequency
  };
}
...
export function Dashboard() {
  ...
  const safeAutomationAnalysis: AutomationAnalysis = {
    ...
    automationTasks: ((automationAnalysis?.automationTasks ?? []).map(task => ({
      ...
      processSummary: (task as any).processSummary
    ...
    recurringTasks: ((automationAnalysis?.recurringTasks ?? []).map(task => ({
      ...
      exactMatches: task.exactMatches,
      similarMatches: task.similarMatches,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          ...
          <div className="text-sm text-gray-500">
            Analyzed {filteredTasks.length} tasks in this time period
          </div>
        </div>
      </div>
